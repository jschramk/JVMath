...
public class Rule<T> {
  ...
  public static String convertToCode(JsonObject jsonObject, Parser parser) throws ParserException {
    ...
    if (jsonObject.has(REQUIRE)) {
      ...
      for (JsonElement ele : reqs) {
        ...
        if (unsupportedKeys.size() > 1) {
          throw new IllegalArgumentException("ERROR: Unsupported keys: " + unsupportedKeys);
        }
