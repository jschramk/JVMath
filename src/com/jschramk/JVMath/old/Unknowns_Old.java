...
public class Unknowns_Old {
  ...
  public void removeKnown(Knowns_Old knownsOld) {
    Iterator<Map.Entry<Operand, Unknown>> iterator = unknownMap.entrySet().iterator();
    while (iterator.hasNext()) {
      Map.Entry<Operand, Unknown> entry = iterator.next();
      entry.getValue().instances.entrySet().removeIf(operandIdentityHashSetEntry -> knownsOld
  ...
  public void removeUsed(Knowns_Old knownsOld) {
    for (Map.Entry<Operand, Unknown> entry : unknownMap.entrySet()) {
      for (Map.Entry<Operand, IdentityHashSet<Operand>> instance : entry.getValue().instances
          .entrySet()) {
        instance.getValue().removeIf(operand -> knownsOld.getUsedIds().contains(operand.getId()));
      }
    }
  }

  public void removeImpossible(Knowns_Old knownsOld) {
      ...
      for (Map.Entry<Operand, IdentityHashSet<Operand>> instance : entry.getValue().instances
        ...
        instance.getValue().removeIf(choice -> {
          ...
          boolean knownMismatch =
              knownsOld.containsGeneral(unknown) && !knownsOld.getGeneral(unknown).equals(choice);

          Operand parent = unknown.getParent();

          boolean notChildOfParent = parent != null && knownsOld
              .containsInstance(parent) && !knownsOld
          ...
          if (unknown.hasChildren()) {
            ...
            for (Operand child : unknown) {
              ...
              if (knownsOld.containsInstance(child) && !choice
                  .hasChildInstance(knownsOld.getInstance(child))) {
          ...
          if (DEBUG && (knownMismatch || notChildOfParent || notParentOfChild)) {
            ...
            if (knownMismatch) {
              System.out.println(unknown + " = " + knownsOld.getGeneral(unknown));
            } else if (notChildOfParent) {

              Operand analogParent = knownsOld.getInstance(unknown.getParent());
  ...
  public boolean makeNextChoice(Knowns_Old knownsOld) {
    ...
    for (int i = 0; i < assignList.size(); i++) {
      ...
      if (i == assignList.size() - 1) {
        ...
        if (choiceList.size() == assignList.size() || sameOperand) {
          knownsOld.add(assignList.get(i), choiceList.get(i));
        } else {
          ...
          if (assign.getType() != parentType && assign.getType() != Operand.Type.VARIABLE) {
            knownsOld.add(assign, choiceList.get(i));
            continue;
          }
          ...
          if (parent instanceof BinaryOperation
            ...
            knownsOld.add(assignList.get(i), operation);
            knownsOld.addUsed(leftover);
  ...
  // returns false iff mapping is impossible
  public boolean mapAll(Knowns_Old knownsOld) {
    ...
    for (int i = 0; i < MAX_ITERATIONS; i++) {
      ...
      removeUsed(knownsOld);
      removeKnown(knownsOld);
      removeImpossible(knownsOld);
      applyFilters();
      ...
      if (!makeNextChoice(knownsOld)) {

        timer.stop();

        //timer.printDelta("Reject time:");
